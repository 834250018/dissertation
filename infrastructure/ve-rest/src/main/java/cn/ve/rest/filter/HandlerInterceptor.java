...
public class HandlerInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
        ...
        boolean ignoreAuthCheck = handlerMethod.getMethod().isAnnotationPresent(PermitAll.class);
        if (ignoreAuthCheck) {
            return Boolean.TRUE;
        }
    ...
    private void checkAuth(HttpServletRequest request) {
        String token = getFromRequest(request, "authorization");
        if (StringUtils.isBlank(token)) {
            throw new VeBaseException("401");
        }
